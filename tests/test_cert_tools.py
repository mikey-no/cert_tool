...
def test_self_signed_server(tmp_path):
    ...
    web_server_process_handle = tls_web_server_process(
        cert_tool.cert_file, cert_tool.private_key_file
    )
...
def test_ca_signed_server(tmp_path):
    ...
    assert (
        str(cert_tool_leaf.csr.subject) == f"<Name(C={cert_tool_leaf.COUNTRY_NAME},"
    ...
    assert (
        str(cert_tool_leaf.cert.subject) == f"<Name(C={cert_tool_leaf.COUNTRY_NAME},"
        f"ST={cert_tool_leaf.STATE_OR_PROVINCE_NAME},"
        f"L={cert_tool_leaf.LOCALITY_NAME},"
        f"O={cert_tool_leaf.ORGANIZATION_NAME},"
        f"CN={prefix}-{common_name})>"
...
def test_ca_signed_server_with_private_key_encryption_bad_password(tmp_path):
    ...
    cert_tool_root.save_private_key(password=password)
    cert_tool_root = (
        None  # remove the existing object to check the private in a file works
    )
    cert_tool_root = CertTool(
        location=tmp_path,
        common_name="root cert",
...
def test_ca_signed_server_with_mtls(tmp_path):
    ...
    assert (
        str(cert_tool_client.csr.subject) == f"<Name(C={cert_tool_client.COUNTRY_NAME},"
    ...
    assert (
        str(cert_tool_client.cert.subject)
        == f"<Name(C={cert_tool_client.COUNTRY_NAME},"
        f"ST={cert_tool_client.STATE_OR_PROVINCE_NAME},"
        f"L={cert_tool_client.LOCALITY_NAME},"
        f"O={cert_tool_client.ORGANIZATION_NAME},"
        f"CN={prefix}-{common_name})>"
...
def test_main_root_and_main_leaf(tmp_path):
    ...
    root_ca = create_root(
        prefix=prefix, root_ca_common_name=root_ca_common_name, location=tmp_path
    )
    ...
    leaf_cert_tool.cert = sign_csr(
        leaf_cert_tool.csr_file,
        prefix=prefix,
        location=tmp_path,
    ...
    log.info("running web server with certs")
    web_server_process_handle = tls_web_server_process(
        leaf_cert_tool.cert_file, leaf_cert_tool.private_key_file
    )
    next(web_server_process_handle)  # use next to use the yielded iterator
    log.info("testing the web server and certs")
    r = requests.get(
...
def test_main_root_and_main_leaf_with_san(tmp_path):
    ...
    # remove duplicates
    out_list = list(
        dict.fromkeys(leaf_cert_tool.cert_info()["subject_alternate_names"])
    )
