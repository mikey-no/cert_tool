...
from app import utils  # needed to configure the logging
...
def parse_args() -> argparse:
    ...
    parser.add_argument(
        ...
        dest="location",
        type=pathlib.Path,
        help=f"location to store certs and keys, defaults to: {default_location}",  # set this in CertTool, not here
    )
    ...
    parser.add_argument("-pwd", "--password", type=str, help="private key password")
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="list of zero of more subject alternate names, some names like localhost are added "
        "automatically, must be used with --sign_csr argument, does not understand ip addresses",
    )
...
def create_root(
    ...
    cert_tool_root = CertTool(
        location=location, common_name=root_ca_common_name, prefix=prefix
    )
...
def re_use_existing_ca(
    prefix: str, location: pathlib.Path, root_ca_common_name: str = root_ca_common_name
):
    """
    ...
    if dummy_root_ca.private_key_file.exists():
        ...
        log.critical(
            f"unable to find the root ca private key file: {dummy_root_ca.private_key_file}"
        )
        sys.exit(1)
...
def sign_csr(
    ...
    if sign_csr_file.exists():
        ...
        pseudo_leaf = CertTool(
            prefix=prefix, common_name="leaf_temp_common_name", location=location
        )
        pseudo_leaf.csr_file = sign_csr_file
        pseudo_leaf_load_csr_result = (
            pseudo_leaf.load_csr()
        )  # use this to turn the csr in file to a csr
        pseudo_leaf.load_common_name_from_csr()
        # need to load up the private key from the root ca to enable this cert to e signed
        re_used_root_ca = re_use_existing_ca(
            prefix=prefix, location=location, root_ca_common_name=root_ca_common_name
        )
        if re_used_root_ca.private_key_file.exists() is False:
            ...
            if pseudo_leaf_load_csr_result is not None:
                pseudo_leaf.cert = re_used_root_ca.sign_certificate(
                    pseudo_leaf.csr, subject_alternate_name
                )
                ...
                log.error(
                    f"Unable to sign the csr the cert was not loaded: {pseudo_leaf_load_csr_result}"
                )
                return None
    else:
...
def main():
    ...
    if args.create_root and args.sign_csr:
        log.critical(
            "Cannot create a root ca and sign a csr at the same time (optionally use with --san)"
        )
        sys.exit(-1)

    if args.create_root is False and args.sign_csr is None:
        log.critical(
            "Create a root ca (--create_root) or sign a csr (--sign_csr), pick one"
        )
        sys.exit(-1)

    if args.san and args.create_root:
        log.critical(
            "Cannot set a list of subject alternate names until the cert is being signed, only use with "
            "sign_csr"
        )

    if args.create_root:
        create_root(
            args.prefix, location=args.location, root_ca_common_name=root_ca_common_name
        )
        sys.exit(0)

    if args.sign_csr:
        sign_csr(
            sign_csr_file=args.sign_csr,
            prefix=args.prefix,
            location=args.location,
            subject_alternate_name=args.san,
        )
        info_cert_tool = CertTool(
            prefix=args.prefix, common_name=socket.getfqdn(), location=args.location
        )
