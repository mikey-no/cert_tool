...
class CertTool:
    ...
    def __init__(self,
        ...
        if common_name is None:
            self.common_name = socket.getfqdn()  # may be replaced by calling self.set_common_name
        else:
        ...
        if isinstance(private_key_encryption_password, str):
            self.private_key_encryption_password = bytes(private_key_encryption_password, encoding='utf-8')
        else:
            self.private_key_encryption_password = private_key_encryption_password

        if self.use_private_key_encryption:
            log.info('Private key will be encrypted')
        else:
            log.info('Private key will not be encrypted')

        if location is None:
            self.set_location(pathlib.Path.cwd() / prefix)
        else:
    ...
    def set_location(self, location: pathlib.Path | str):
        """ must set the common_name, prefix and location for a valid CertTool operation """
        if isinstance(location, str):
            self.location = pathlib.Path(location) / self.prefix
        elif isinstance(location, pathlib.Path):
            self.location = location / self.prefix
        else:
            log.critical('Trying to create the cert tool files in an invalid location')
            sys.exit(-1)
    ...
    def _set_file_names(self):
        ...
        try:
            ...
            self.cert_tool_info_file: pathlib.Path = self.location / f'{self.common_name}_cert_tool_info.json'
    ...
    def create_private_key(self):
        ...
        if self.load_private_key(password=self.private_key_encryption_password):
            log.info(f'Not created a new private key: {self.common_name}, have one in a file')
        else:
            self.private_key = ec.generate_private_key(self._key_type(), default_backend())
            self.public_key = self.private_key.public_key()
            log.info(f'Private key created for common name: {self.common_name}, ec type: {ec.SECP256R1.name}')
        return None
    ...
    def save_private_key(self, password: str | bytes | None = None):
        ...
        if self.use_private_key_encryption and password is None:
            log.critical(f'No password supplied for private key with common name: {self.common_name}')
            sys.exit(-1)

        if self.private_key_file.exists():
            backup_file = self.private_key_file.parent / f'{self.private_key_file.name}.bak'
            log.info(f'Making a backup of private key: {self.common_name} in:  {backup_file}')
        ...
        if self.use_private_key_encryption:
            with open(self.private_key_file, "wb") as f:
                ...
                encryption_algorithm = serialization.BestAvailableEncryption(password=password_bytes)
                f.write(
        ...
        log.info(f'Private key: {self.common_name} saved to file: {self.private_key_file}')
        ...
        if self.private_key_file.exists():
            if self.use_private_key_encryption:
                if self.private_key_encryption_password is None:
                    if password is None:
                        password = input(f'Please provide the password to open private key with common name: '
                                         f'{self.common_name}: ')
                else:
                ...
                with open(self.private_key_file, "rb") as f:
                    self.private_key = load_pem_private_key(f.read(), password=password_bytes)
            else:
                with open(self.private_key_file, "rb") as f:
            ...
            if isinstance(self.private_key, EllipticCurvePrivateKey) is False:
                log.critical(f'Private key for: {self.common_name}, was invalid: {type(self.private_key)}')
                sys.exit(-1)
        else:
            return False
        return True
    ...
    def load_csr(self):
        ...
        with open(self.csr_file, 'rb') as f:
            ...
            if isinstance(data_pem, x509.CertificateSigningRequest):
                self.csr = data_pem
                log.info(f'CSR is loaded, signature is valid: {self.csr.is_signature_valid}')
            else:
    ...
    def _get_subject_(self):
        return x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, self.ORGANIZATIONAL_UNIT_NAME),
    ...
    def sign_certificate(self,  csr: x509.CertificateSigningRequest):
        ...
        log.info(f'{subject.rfc4514_string()} is signing the certificate for {csr.subject} csr')
    ...
    def save_cert_tool(self) -> None:
        ...
        def my_converter(obj):
            ...
            if isinstance(obj, EllipticCurvePrivateKey):
                if self.use_private_key_encryption:
                    return b64(obj.private_bytes(encoding=serialization.Encoding.PEM,
                                                 format=serialization.PrivateFormat.TraditionalOpenSSL,
                                                 encryption_algorithm=
...
def ca_certs_tls_recipe() -> None:
    ...
    prefix = 'dev_tls'
    cert_tool_root = CertTool(location=cert_location, common_name='root cert', prefix=prefix)
    ...
    cert_tool_leaf = CertTool(location=cert_location, common_name=common_name, prefix=prefix)
...
def ca_certs_tls_recipe_private_key_encryption() -> None:
    ...
    prefix = 'dev_tls'
    password = '1234'
    cert_tool_root = CertTool(location=cert_location,
    ...
    cert_tool_root.create_private_key()
    cert_tool_root.save_private_key(password=password)  # added to test private key life cycle
    cert_tool_root = None
    cert_tool_root = CertTool(location=cert_location,
                              common_name='root cert',
                              prefix=prefix,
...
def ca_certs_mtls_recipe() -> None:
    ...
    cert_tool_client = CertTool(location=cert_location, common_name=common_name, prefix=prefix)
